...
// ── 2a. tariff title freshness ───────────────────────────────────────────────
// A stale "2024-25" in page titles undercuts the visible "checked against the official order"
// promise before a reader opens the source.
console.log('\n• refreshed tariff page titles avoid stale FY 2024-25 copy');
{
  const watched = /(Manipur|MSPDCL|Arunachal|APDOP|Dadra|Daman|Diu|DNHDDPDCL|Ladakh|LPDCL|Mizoram|Nagaland)/i;
  const stale = [];
  for (const page of htmlPages) {
    const html = fs.readFileSync(path.join(ROOT, page), 'utf8');
    const title = /<title>([^<]+)<\/title>/.exec(html)?.[1] || '';
    if (watched.test(title) && /2024-25/.test(title)) stale.push(`${page}: ${title}`);
  }
  if (stale.length) fail('refreshed tariff title year is stale:\n    ' + stale.join('\n    '));
  else { passed++; console.log('  ✓ no refreshed state title advertises FY 2024-25'); }
}
...
// ── 8b. tariff database page names the latest refresh honestly ───────────────
// The public database page is where readers sanity-check the site's currency. It should show
// the latest verification batch, and it must distinguish public proposal evidence from final
// approved tariff orders.
console.log('\n• tariff database page refresh summary');
{
  const page = path.join(ROOT, 'database', 'index.html');
  if (!fs.existsSync(page)) {
    fail('database/index.html missing — run `npm run seo`');
  } else {
    const html = fs.readFileSync(page, 'utf8');
    ...
    const problems = [];
    if (!html.includes('Latest database refresh')) problems.push('missing latest database refresh section');
    for (const state of expectedStates) {
      if (!html.includes(state)) problems.push(`latest refresh section missing ${state}`);
    }
    if (!html.includes('FY 2026-27 proposal pending')) problems.push('missing Nagaland pending-proposal label');
    if (html.includes("regulator's own order")) problems.push('still overclaims every linked source as a regulator order');

    if (problems.length) {
      for (const p of problems) fail(`database/index.html: ${p}`);
    } else {
      passed++;
      console.log('  ✓ database page shows the latest refresh and labels pending public updates');
    }
  }
}
