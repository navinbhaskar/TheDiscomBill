...
// Bill Month renders as a name, never "07" — shared by both bill renderers.
...
/**
 * Render Simple mode's result: the same mini-bill anatomy as the homepage hero card —
 * total, one row per charge, the slab working under Energy, a rule above the total.
 *
 * Simple mode asks for five things (state, DISCOM, category, units, load). The full
 * renderBill() below answers with a 28-row utility-bill facsimile whose Consumer Details
 * block — name, account no., address, meter no. — is em-dashes, because Simple mode never
 * collected any of it. This renders only what the visitor actually gave us; the full
 * facsimile is one click away and stays the Detailed-mode output.
 *
 * @param {Object} params - The engine result plus the billing month/year.
 * @returns {string} Bill HTML.
 */
export function renderSimpleBill({ result, billingMonth, billingYear }) {
  const {
    discom, category, supplyTypeName, units, energyUnit,
    connectedLoadKw, demandUnit, billingDemand, fixedCharge, fixedChargeConfig,
    slabBreakdown, totalEnergy, wheelingCharge, wheelingLabel,
    excessDemandPenalty, minChargeTopUp, todPeakSurcharge, todOffPeakRebate,
  ...
  if (wheelingCharge) lines.push(row(wheelingLabel || 'Wheeling charge', wheelingCharge));
  if (todPeakSurcharge) lines.push(row('TOD peak surcharge', todPeakSurcharge));
  if (todOffPeakRebate) lines.push(row('TOD off-peak rebate', -Math.abs(todOffPeakRebate), { cls: 'mini-line-credit' }));
  if (excessDemandPenalty) lines.push(row('Excess demand penalty', excessDemandPenalty));
  if (minChargeTopUp) lines.push(row('Minimum charge top-up', minChargeTopUp));
  if (facAmount) {
    const rate = facMode === 'percent' ? `${facRate > 0 ? '+' : '−'}${Math.abs(facRate)}%` : `₹${Math.abs(facRate)}/unit`;
    lines.push(row(facAmount < 0 ? 'FPPA credit' : 'FPPA (fuel surcharge)', facAmount,
      { detail: rate, cls: facAmount < 0 ? 'mini-line-credit' : '' }));
  }
  for (const c of (extraCharges || [])) if (c.amount) lines.push(row(c.name, c.amount));
  if (subsidyAmount) lines.push(row(subsidyLabel || 'Subsidy', -Math.abs(subsidyAmount), { cls: 'mini-line-credit' }));

  // Real bills carry a rounding line, and without it the column visibly does not close:
  // currentNet is the rounded figure, currentGross − subsidy is what the rows above sum to.
  const rounding = +(currentNet - (currentGross - (subsidyAmount || 0))).toFixed(2);
  if (Math.abs(rounding) >= 0.01) lines.push(row('Rounding', rounding, { cls: 'mini-line-quiet' }));

  // Arrears/payments/adjustments live in Detailed mode's tabs, so they are normally absent
  // here — but a shared link can carry them into Simple mode, and silently dropping a
  // charge from a bill would be worse than an extra row.
  const extras = [];
  const arrearTotal = Number(arrears) || 0;
  if (arrearTotal) extras.push(row('Arrears', arrearTotal));
  ...
  const periodBits = [
    ...
    `${units} ${energyUnit || 'units'}`,
    connectedLoadKw ? `${connectedLoadKw} ${demandUnit || 'kW'}` : '',
  ].filter(Boolean).join(' · ');
  ...
  <div class="bill-wrap simple-bill">
    ...
    <dl class="mini-lines sb-lines">
      ${lines.join('')}
      ${extras.length ? `
        <div class="mini-line sb-line mini-line-total"><dt>Current bill</dt><dd>${formatINR(currentNet)}</dd></div>
        ${extras.join('')}` : ''}
      <div class="mini-line sb-line mini-line-total">
