...
'ql.advancedCalc': 'Advanced Bill Calculator',
    ...
    'footer.newConnection': 'New Connection',
    ...
    'hero.stat.records': 'Tariff records',
    ...
    'states.region.north': 'North India',
    'states.region.south': 'South India',
    'states.region.west': 'West India',
    'states.region.central': 'Central India',
    'states.region.east': 'East India',
    'states.region.northeast': 'North-East India',
    'states.discom': 'DISCOM',
    'states.discoms': 'DISCOMs',
    'states.perUnit': '/unit',
    'trust.title': 'Your trusted electricity bill calculator',
    'trust.orders': 'Rates taken from published SERC tariff orders — <a href="/orders/">every source listed</a>',
    'trust.free': 'Free to use — every calculator works without an account',
    'trust.local': 'Your bill is calculated in your browser; a photo goes to OCR only if you allow it',
    'cap.title': 'Every charge on your bill, modelled',
    'discoms.popular': 'Popular DISCOMs',
    'discoms.viewAll': 'View all 65 →',
    ...
    'hero.card.energy': 'Energy charge',
    'hero.card.fixed': 'Fixed charge',
    'hero.card.duty': 'Electricity duty',
    'hero.card.credit': 'FPPA credit',
    'hero.card.ppac': 'Fuel surcharge (PPAC)',
    'hero.card.fac': 'Fuel surcharge (FAC)',
    'hero.card.wheeling': 'Wheeling charge',
    'hero.card.none': 'none notified',
    'hero.card.noneLevied': 'none levied',
    'hero.card.rounding': 'Rounding',
    'hero.card.totalPayable': 'Total payable',
    'hero.card.foot': 'Live engine · straight from the tariff order',
    'hero.card.sub0': '250 units · 2 kW · LMV-1 domestic',
    'hero.card.sub1': '250 units · 2 kW · LT-I domestic',
    'hero.card.sub2': '250 units · 2 kW · LT-1 domestic',
    'hero.card.sub3': '250 units · 2 kW · LT-I(B) residential',
    ...
    'label.purpose': 'What is the connection for?',
    'purpose.home': 'Home',
    'purpose.business': 'Shop / Business',
    ...
    'sol.billAmt': 'Your average monthly electricity bill (₹)',
    'sol.phBillAmt': 'e.g., 2450',
    'sol.billAmtHint': "We'll work back to your units using the tariff below.",
    ...
    'sol.advanced': 'Assumptions',
    'sol.cost': 'System cost (₹ per kW)',
    'sol.costHint': 'Turnkey installed cost before subsidy. Typically ₹50,000–65,000/kW.',
    'sol.empty': 'Enter your monthly bill to see your solar estimate.',
    'sol.heroLabel': 'Net savings over 25 years',
    'sol.heroSystem': 'system',
    'sol.monthlyHint': 'Set this if you know it — it overrides the figure worked back from your bill.',
    'sol.paysFor': 'pays for itself in',
    'sol.gen': 'Yearly units generated',
    'sol.grossCost': 'Upfront cost',
    'sol.centralSub': 'PM Surya Ghar subsidy',
    'sol.stateSub2': 'State subsidy',
    'sol.netCost': 'Net cost to you',
    'sol.monthlySave': 'Yearly savings',
    'sol.payback': 'Payback period',
    ...
    'ev.rCng': 'CNG for the same km',
    'ev.cngMileage': 'CNG mileage (km/kg)',
    'ev.cngHint': 'Varies a lot by city — check your local pump. Set 0 to hide CNG.',
    'ev.cng': 'CNG price (₹/kg)',
    ...
    'ev.chartTitle': 'Monthly cost compared',
    ...
    'about.p1b': 'Beyond the calculator, TheDiscomBill publishes <a href="/tariffs/">current tariff rates for every state</a>, a <a href="/smart-meter/">guide to reading your smart meter</a>, an <a href="/check-my-bill/">instant self-check that reads your bill photo</a>, and calculators for <a href="/solar-calculator/">rooftop solar</a> and <a href="/ev-charging-calculator/">EV charging</a> — all free, in English, हिंदी, मराठी and தமிழ்.',
    ...
    'about.howTitle': 'How the calculator works',
    'about.p2': 'The calculator uses the latest publicly available tariff orders from each DISCOM. It computes charges using the <strong>slab-wise (cumulative) method</strong>: the rate for each slab applies only to the units consumed within that slab, not to the entire consumption. Fixed / demand charges are applied separately based on your connected (sanctioned) load.',
    ...
    'about.disclaimer': '<strong>⚠ Estimate, not a bill:</strong> TheDiscomBill applies the tariff order your State Electricity Regulatory Commission has published, using the same slab-wise method your DISCOM uses, and the engine is checked against real consumer bills — see <a href="/methodology/">methodology &amp; accuracy</a> for sourcing and verification. Even so, it cannot see what is specific to your connection: meter reading dates and the length of your billing cycle, arrears and previous adjustments, security-deposit interest, rebates, or local surcharges — so your DISCOM’s figure may differ. A gap of a few rupees is rounding; a large gap is worth questioning, and <a href="/guides/bill-wrong-how-to-complain-and-win/">this guide shows how to raise it</a>. Rates here are updated as new tariff orders are notified, and your DISCOM’s bill remains the official one. TheDiscomBill is independent and not affiliated with any electricity utility or government body.',
    // Breadcrumb (shared)
    'bc.home': 'Home',
    ...
    'cmp.th.first': 'First slab',
    'cmp.th.top': 'Highest slab',
    'cmp.th.fixed': 'Fixed',
    'cmp.th.free': 'Free units',
    'cmp.th.u200': '200 Units',
    'cmp.th.u500': '500 Units',
...
// twin pages, and `font` names the self-hosted stylesheet under /fonts/ carrying that
// script's face (loaded on demand by ensureLangFont; null = covered by the base Latin
// faces). Adding a language = a STRINGS dict + a row here; the switcher menu on every
// page is built from this list.
export const LANGS = [
  { code: 'en', name: 'English', badge: 'EN',  hreflang: 'en-IN', font: null },
  { code: 'hi', name: 'हिंदी',    badge: 'हिं',  hreflang: 'hi-IN', font: 'noto-devanagari' },
  { code: 'mr', name: 'मराठी',   badge: 'म',   hreflang: 'mr-IN', font: 'noto-devanagari' },
  { code: 'ta', name: 'தமிழ்',   badge: 'த',   hreflang: 'ta-IN', font: 'noto-tamil' },
];
...
// activated — so English visitors never download them. Injected once per stylesheet;
// the CSS stack already lists the Indic families after the Latin faces, so the
// glyphs are picked up per-glyph.
//
// Self-hosted (fonts/noto-*.css + the matching variable woff2, generated by
// fonts/_localize-indic.mjs) — the whole site avoids Google Fonts, and these used to
// be the one runtime exception. One sheet per script so Hindi/Marathi visitors don't
// pull the Tamil face, or vice versa.
const fontsRequested = new Set();
function ensureLangFont(lang) {
  const sheet = langMeta(lang).font;
  if (!sheet || fontsRequested.has(sheet)) return;
  fontsRequested.add(sheet);
  const link = document.createElement('link');
  link.rel = 'stylesheet';
  link.href = `/fonts/${sheet}.css`;
